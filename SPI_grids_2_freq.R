...
negspi_list2<-c("0","_neg1_","_neg15_","_neg2_")
...
for (i in 1:length(spi_list)) { #loop through all 10 SPI time periods
  ...
  for (k in 1:9){
    ...
    rNA1<-mask(rNA0,rfa_mask2,filename=fname1,overwrite=TRUE)
    rNA15<-mask(rNA0,rfa_mask2,filename=fname15,overwrite=TRUE)
    rNA2<-mask(rNA0,rfa_mask2,filename=fname2,overwrite=TRUE)
