...
for (i in 1:length(spi_list)) { #loop through all 10 SPI time periods
  ...
  for (k in 1:9){
    ...
    #save as tif
    tname<-paste("SPI",spitime,"Mn_",decade,".tif",sep="")
    writeRaster(spi_dec_mean,filename=tname,overwrite=TRUE)
  
  }
} #next SPI time period 
  

#PLOT ALL MEAN IMAGES BY DECADE IN ONE FIGURE:
#spi_list<-c(1,3,6,9,12,18,24,36,48,60) #10 spi time periods

for (i in 1:length(spi_list)) { #loop through all 10 SPI time periods and save images:
  #Set SPI time period (3-month, 6-month, etc.)
  spitime<-spi_list[i]
  ...
  plt<-spplot(s4,col.regions=pal5,equal=TRUE,at=brks1,main=paste("Mean SPI-",spitime," By Decade",sep=""),names.attr=c("1920-1929","1930-1939","1940-1949","1950-1959","1960-1969","1970-1979","1980-1989","1990-1999","2000-2012"))+layer(sp.polygons(coast))
  print(plt)
  dev.off()
